...
```bash
make tools && ./tools/servo_test
```

펄스폭(µs)을 직접 조작해 가동 범위와 회전 방향을 실측한다. 각도 변환을 거치지
않는 이유는, 각도 변환에 쓰이는 `min_pulse_ms`/`max_pulse_ms` 자체가 지금
확인하려는 대상이기 때문이다. 시작 시 양쪽을 1500µs(중앙)로 보낸다 — 현재 각도를
알 수 없으므로 어느 쪽 끝에서든 안쪽으로 들어오는 중앙이 가장 안전하다.

**방향은 몸이 아니라 화면 기준으로 판단할 것.** "내가 왼쪽에 섰다"는 카메라 영상에서
오른쪽일 수 있다. 확실한 방법은 `--log-level DEBUG` 로 추적 로그를 보고 `err` 이
0 으로 줄어드는지 확인하는 것이다:

```
box=(64,120)/320x240  err=(-0.60,+0.00)  d=(-6.0,+0.0)  now=(+0.0,+0.0)
```

`err` 이 커지면 서보가 반대로 도는 것 → 해당 축의 `invert` 를 뒤집는다.
...
핀 선택에 제약은 없다. pigpio 의 `set_servo_pulsewidth` 는 칩의 하드웨어 PWM
채널(PWM0/PWM1)이 아니라 DMA 로 타이밍을 만들기 때문에 아무 GPIO 나 쓸 수 있다.
GPIO12/13 을 고른 건 나란히 붙어 있고 GND 가 가까워서다.
...
| `motor.min_pulse_ms/max_pulse_ms` | 서보 펄스폭 범위. MG90S 실측 0.5~2.5ms |
| `motor.pan_min/max`, `tilt_min/max` | 기구적 가동 범위(deg). 실측 ±90 |
